...
def test_leo_geo_time_history():
    ...
    edelbaum_accel = guidance_law(k, a_0, a_f, i_f - i_0, f)
    ...
    r, v = cowell(k,
                  ...
                  t_f,
                  #dt=0.1,  # To integrate in a loop
                  ad=edelbaum_accel,
                  #nsteps=1000000)  # NEVER FINISHES
                  nsteps=10000000,  # Does not converge
                  rtol=1e-5)
    ...
    assert_almost_equal(sf.a.to(u.km).value, a_f)
    assert_almost_equal(sf.inc.to(u.deg).value, i_f)
    assert_almost_equal(sf.ecc.value, 0.0)
